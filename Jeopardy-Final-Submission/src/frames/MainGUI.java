...
public class MainGUI extends JFrame {
	...
	protected JPanel [] clockLabelPosition;
	...
	public MainGUI(User loggedInUser){
		...
		myName = loggedInUser.getUsername();
	}
	...
	private JPanel createProgressPanel() {
		...
		JLabel updatesLabel = new JLabel("Game Progress     " + myName);
		...
		// hold for clock labels
		clockLabelPosition = new JPanel [3];
		...
		for (int i = 0; i < gameData.getNumberOfTeams(); i++) {
			clockLabelPosition[i] = new JPanel();
			clockLabelPosition[i].setBackground(Color.darkGray);
			//clockLabelPosition[i].setBorder(BorderFactory.createLineBorder(Color.white));
			
			TeamGUIComponents team = gameData.getTeamDataList().get(i);
			pointsPanel.add(team.getMainTeamNameLabel());
			pointsPanel.add(clockLabelPosition[i]);
